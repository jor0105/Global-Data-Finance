...
REVIEWER_SUPPORT_ROOT = AGENTS_ROOT / 'runtime' / 'reviewer'
SKILLS_ROOT = AGENTS_ROOT / 'skills'
LINT_AND_VALIDATE_SKILL_ROOT = SKILLS_ROOT / 'lint-and-validate'
REVIEW_SESSION_ROOT = SKILLS_ROOT / 'review-session'
REVIEW_ITEM_ROOT = SKILLS_ROOT / 'review-item'
REVIEW_CLOSEOUT_ROOT = SKILLS_ROOT / 'review-closeout'

TEMPLATE_PATHS = {
    'review-session.template.md': (
        REVIEW_SESSION_ROOT / 'templates' / 'review-session.template.md'
    ),
    'review-plan.template.md': (
        REVIEW_SESSION_ROOT / 'templates' / 'review-plan.template.md'
    ),
    'finding.template.md': REVIEW_ITEM_ROOT
    / 'templates'
    / 'finding.template.md',
    'gate-report.template.md': (
        LINT_AND_VALIDATE_SKILL_ROOT / 'templates' / 'gate-report.template.md'
    ),
    'security-handoff.template.md': (
        REVIEW_ITEM_ROOT / 'templates' / 'security-handoff.template.md'
    ),
    'verdict.template.md': REVIEW_CLOSEOUT_ROOT
    / 'templates'
    / 'verdict.template.md',
    'review-summary.template.md': (
        REVIEW_CLOSEOUT_ROOT / 'templates' / 'review-summary.template.md'
    ),
}

ARTIFACT_SCHEMA_PATHS = {
    'review-session': REVIEW_SESSION_ROOT
    / 'schemas'
    / 'review-session.schema.json',
    'review-plan': REVIEW_SESSION_ROOT / 'schemas' / 'review-plan.schema.json',
    'finding': REVIEW_ITEM_ROOT / 'schemas' / 'finding.schema.json',
    'gate-report': LINT_AND_VALIDATE_SKILL_ROOT
    / 'schemas'
    / 'gate-report.schema.json',
    'security-handoff': (
        REVIEW_ITEM_ROOT / 'schemas' / 'security-handoff.schema.json'
    ),
    'verdict': REVIEW_CLOSEOUT_ROOT / 'schemas' / 'verdict.schema.json',
}
...
PACK_REGISTRY = json.loads(
    (REVIEWER_SUPPORT_ROOT / 'pack-registry.json').read_text(encoding='utf-8'),
)
...
def resolve_template_path(template_name: str) -> Path:
    try:
        ...
        raise ArtifactError(f'unknown template: {template_name}') from error
    if not template_path.exists():
        raise ArtifactError(f'missing template: {template_path}')
    return template_path


def artifact_schema_path(artifact_type: str) -> Path:
    try:
        schema_path = ARTIFACT_SCHEMA_PATHS[artifact_type]
    except KeyError as error:
        raise ArtifactError(
            f'unknown schema for artifact type: {artifact_type}'
        ) from error
...
def validate_artifact(payload: dict[str, Any]) -> None:
    ...
    artifact_schema_path(artifact_type)
    if artifact_type == 'review-session':
        validate_review_session(payload)
        return
    if artifact_type == 'review-plan':
        validate_review_plan(payload)
        return
    if artifact_type == 'finding':
    ...
    if artifact_type == 'verdict':
        ...
        return
    if artifact_type == 'security-handoff':
        validate_security_handoff(payload)
        return
...
def assert_transition(
    ...
    transitions = {
        'review-session': {
            ...
            'planned': {'reviewing', 'apply-ready', 'blocked'},
            'reviewing': {
                'awaiting_security',
                'apply-ready',
                'completed',
...
def detect_packs(changed_files: list[str], security_touch: str) -> list[str]:
    ...
    for file_path in changed_files:
        ...
        if any(
            token in lower
            for token in ('api', 'route', 'controller', 'http', 'service')
        ):
            matched.add('api')
        if any(
            token in lower
            ...
            matched.add('data')
        if (
            lower.endswith(('.test.ts', '.test.tsx', '.spec.ts', '.spec.tsx'))
            or '/tests/' in lower
        ):
            matched.add('tests')
        if lower.endswith(('.tsx', '.jsx', '.css')) or '/src/' in lower:
