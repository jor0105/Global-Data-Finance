...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Exporta um resumo humano reproduzivel do review.'
    )
...
def main() -> int:
    ...
    summary = render_template(
        'review-summary.template.md',
        {
            ...
            'planProgress': f'{session["statistics"]["completedItems"]}/{session["statistics"]["totalItems"]}',
            'gateSummary': [
                f'- {gate["gateId"]}: {gate["status"]} ({"blocking" if gate["blocking"] else "advisory"})'
                for gate in gates['gates']
            ]
            or ['- nenhum gate executado'],
            'findings': [
                f'- {finding["findingId"]} [{finding["severity"]}/{finding["confidence"]}]: {finding["summary"]}'
