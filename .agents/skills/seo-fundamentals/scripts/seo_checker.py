...
def is_closed_beta_shell(project_path: Path) -> bool:
    """Detect SPA-style authenticated shells where SEO is only for public entry files."""
    agents_md = project_path / 'AGENTS.md'
    if not agents_md.exists():
        return False

    try:
        text = agents_md.read_text(encoding='utf-8', errors='ignore').lower()
    except OSError:
        return False

    return 'closed-beta' in text and 'auth is mandatory' in text


def is_authenticated_shell_file(file_path: Path, project_path: Path) -> bool:
    """Skip authenticated runtime internals that are not public landing pages."""
    if not is_closed_beta_shell(project_path):
        return False

    try:
        parts = [p.lower() for p in file_path.relative_to(project_path).parts]
    except ValueError:
        parts = [p.lower() for p in file_path.parts]

    path_key = '/'.join(parts)
    if path_key.startswith('src/app/shell/'):
        return True
...
def is_frontend_project(project_path: Path) -> bool:
    """Check if the project is likely a frontend project."""
    indicators = {
        'package.json',
        'node_modules',
        'next.config.js',
    ...
    # Basic check: if there are any page files outside of skip_dirs
    pages = find_pages(project_path, skip_check=True)
    return len(pages) > 3


def is_page_file(file_path: Path, project_path: Path) -> bool:
    """Check if this file is likely a public-facing page."""
    name = file_path.name.lower()
    stem = file_path.stem.lower()

    if is_authenticated_shell_file(file_path, project_path):
        return False
...
def find_pages(project_path: Path, skip_check: bool = False) -> list:
    ...
    if not skip_check and not is_frontend_project(project_path):
        return []
...
def main():
    ...
    if not pages:
        print('\n[!] No page files found.')
        print(
            '    Looking for: HTML, JSX, TSX in pages/app/routes directories'
        )
