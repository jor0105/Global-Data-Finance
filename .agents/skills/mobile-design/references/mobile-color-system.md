...
> Use este arquivo quando cor muda legibilidade, autonomia de bateria, outdoor visibility ou semântica de estado em mobile.
> Ele complementa os guias de plataforma quando a decisão é sobre percepção, contraste e dark mode.

---

## Quick Start

- Abra este arquivo para decidir contraste, dark mode, cores semânticas e impacto de OLED.
- Extraia daqui regras de legibilidade e comportamento; deixe estilo puramente de plataforma para `platform-ios.md` ou `platform-android.md`.
- Se a dúvida é fonte, tamanho ou scaling, vá para `mobile-typography.md`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| regra rápida de cor mobile | `## Five Quick Rules` | legibilidade e consumo de energia |
| dark mode e OLED | `## 2. OLED Considerations` e `## 3. Dark Mode Design` | cor de fundo, texto e adaptação |
| visibilidade ao ar livre | `## 4. Outdoor Visibility` | contraste e saturação |
| cor semântica | `## 5. Semantic Colors` | erro, sucesso, warning e informação |

## Five Quick Rules

- legibilidade vence branding quando a tela está pequena ou em ambiente externo
- dark mode não é só inverter; texto, superfície e hierarquia precisam ser recalibrados
- OLED favorece superfícies escuras bem escolhidas, não qualquer preto puro
- estados semânticos precisam continuar claros nas duas plataformas
- use este arquivo quando a cor muda percepção; use os arquivos de plataforma quando a convenção do OS manda
