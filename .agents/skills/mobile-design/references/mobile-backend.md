...
> Use este arquivo quando a decisão de backend muda por causa do cliente mobile.
> Backend genérico continua pertencendo a `nodejs-best-practices` e `api-patterns`; aqui o foco é rede ruim, retry, versão, push e sync.

---

## Quick Start

- Abra este arquivo quando o problema for push, sync offline, auth mobile, versioning ou upload/media em ambiente instável.
- Extraia daqui a estratégia mínima de compatibilidade com rede ruim, sessão interrompida e recovery.
- Se a dúvida já é puramente de API genérica, volte para `api-patterns`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| push ou notificações | `## 1. Push Notifications` | tipo de push, token e handler |
| sync offline e conflito | `## 2. Offline Sync & Conflict Resolution` | fila, estratégia de merge e replay |
| payload e performance de rede | `## 3. Mobile API Optimization` | paginação, lote e compactação |
| versioning do app | `## 4. App Versioning` | minimum version e compatibilidade |
| auth mobile | `## 5. Authentication for Mobile` | refresh, reauth silenciosa e storage seguro |

## 1. Mobile Backend Mindset
...
## 2. Common Mobile Backend Anti-Patterns
