...
> Use este arquivo quando a solução precisa soar como iOS de verdade, não apenas funcionar no iPhone.
> Ele resume o que muda a decisão em safe area, tipografia, navegação, componentes e feedback.

---

## Quick Start

- Abra este arquivo para revisar decisões que mudam em iPhone ou iPad: safe area, swipe back, Dynamic Type, componentes e feedback tátil.
- Extraia daqui o que precisa ser semanticamente iOS e o que pode continuar compartilhado.
- Se a dúvida ainda é estrutural de shell, volte para `mobile-navigation.md`. Se a dúvida é Android-first, vá para `platform-android.md`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| revisar regras que mudam a solução em iOS | `## What Changes the Solution on iOS` | onde não vale copiar Android ou web |
| fechar checklist de revisão | `## Review Checklist` | pontos mínimos antes de aprovar a experiência |
| comparar o que pode ser compartilhado | `## Shared vs Divergent` | o que segue comum e o que precisa ficar nativo |

## What Changes the Solution on iOS

- safe area e home indicator afetam conteúdo, CTA fixo e bottom sheet
- swipe back é expectativa forte em stacks comuns
- Dynamic Type e semantic colors mudam legibilidade e acessibilidade
- modal, sheet e segmented control têm comportamento esperado próprio
- haptics e feedback visual devem parecer discretos e precisos

## Review Checklist

- safe area respeitada em topo e base
- keyboard e text scaling não cortam conteúdo
- swipe back e dismiss continuam coerentes
- permissões têm pre-prompt ou contexto claro
- feedback visual ou tátil confirma ações importantes
- componentes parecem iOS por hierarquia, densidade e semântica

## Shared vs Divergent

| Área | Pode ser compartilhada | Precisa divergir no iOS quando |
| --- | --- | --- |
| estrutura de fluxo | sim, se o job-to-be-done for igual | o gesto, modal ou shell nativo muda expectativa |
| tipografia | parcialmente | Dynamic Type ou semantic text roles importam |
| navegação | parcialmente | swipe back, tab bar ou modal presentation precisam soar iOS |
| feedback | parcialmente | haptics e transições dão sinal claro de plataforma |
