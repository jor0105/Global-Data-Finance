...
> Use este arquivo para triagem e correção de gargalos em apps React Native ou Flutter.
> Ele ajuda a separar sinal rápido de investigação profunda sem misturar UI, lista, memória e lifecycle.

---

## Quick Start

- Abra este arquivo quando a pergunta for “por que isso está pesado, janky ou instável em device real?”.
- Extraia daqui: categoria do gargalo, triagem mínima, checklist de release e o caminho RN ou Flutter que vale investigar.
- Se o problema ainda é estrutural de navegação ou gesto, volte para `mobile-navigation.md`. Se for validação, feche com `mobile-testing.md`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| descobrir o tipo de gargalo | `## Performance Triage` | lista, animação, imagem, memória, rede ou lifecycle |
| otimizar lista ou renderização RN | `## 2. React Native Performance` | FlatList, FlashList, memoização e layout |
| otimizar árvore Flutter | `## 3. Flutter Performance` | rebuild strategy, `const`, lists e dispose |
| fechar critérios mínimos antes de release | `## Release Gate` | se a tela ainda pode ir adiante |

## Performance Triage

| Sintoma | Verifique primeiro |
| --- | --- |
| scroll ruim | lista, item renderer, image weight, layout thrash |
| animação pesada | propriedade animada, driver, FPS, overlap com scroll |
| tela lenta para abrir | bundle inicial, requests, parsing, heavy composition |
| memória subindo | subscriptions, dispose, navigation leaks, caches |
| retry ou offline ruim | loading infinito, re-fetch excessivo, state recovery |
| bug após background/resume | lifecycle handling, subscriptions, stale state |

## Release Gate

- testado em device médio ou low-end, não só flagship ou simulator
- scroll principal mantém fluidez aceitável
- animações críticas respeitam orçamento de frame
- subscriptions, listeners e controllers têm cleanup
- estados de retry/offline não viram loop ou tela travada
