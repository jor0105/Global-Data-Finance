...
> Use este arquivo para transformar mudanças mobile em um pacote mínimo de validação.
> Ele cobre o que testar, com qual ferramenta e quais cenários não podem ficar implícitos.

---

## Quick Start

- Abra este arquivo quando a dúvida for qual estratégia de teste cabe à mudança mobile.
- Extraia daqui: matriz mínima de regressão, ferramenta por objetivo e pacote mínimo por tipo de mudança.
- Se a discussão já for sobre layout, navegação ou plataforma, volte para os guias específicos e use este arquivo para fechar a validação.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| montar cobertura mínima | `## Minimum Regression Matrix` | cenários que não podem ficar sem teste |
| escolher ferramenta | `## Tool Map by Objective` | unit, component, integration, E2E ou profiling |
| adaptar o pacote de validação ao tipo de mudança | `## Minimum Validation Packages` | depth proporcional ao risco |
| revisar diferenças entre iOS e Android | `## 4. Platform-Specific Testing` | o que precisa ser exercitado por plataforma |

## Minimum Regression Matrix

| Cenário | O que precisa continuar funcionando |
| --- | --- |
| safe area | conteúdo e CTA não colidem com notch, status bar ou home indicator |
| keyboard avoidance | input focado continua visível e ação principal não some |
| offline/retry | estado offline aparece, ação não se perde e existe recuperação |
| lifecycle/background/resume | app retoma contexto ou reconstrói fluxo sem corromper estado |
| permission prompt | pre-prompt, negação e reentrada por settings fazem sentido |
| deep link / push open | navegação chega ao destino com trilha de retorno coerente |
| platform-specific navigation/back | swipe back, hardware back e dismiss funcionam por plataforma |

## Tool Map by Objective

| Objetivo | Ferramenta principal | Complemento recomendado |
| --- | --- | --- |
| lógica pura | Jest ou testes unitários equivalentes | mock apenas nas boundaries |
| componente isolado | RNTL ou `flutter_test` | assertions de estado e acessibilidade |
| fluxo com providers / navegação | integration tests | mocks controlados de APIs e storage |
| jornada crítica real | Detox ou Maestro | device real para pelo menos uma passagem |
| gargalo de desempenho | Flashlight, DevTools, Instruments, Android Profiler | teste em device médio ou low-end |

## Minimum Validation Packages

| Tipo de mudança | Pacote mínimo |
| --- | --- |
| novo fluxo | component + integration + 1 E2E crítico + matriz de platform differences |
| revisão de bug visual | repro manual guiada + component test quando cabível + safe area/keyboard check |
| mudança de navegação | integration ou E2E + back behavior + deep link return |
| offline / permissions / lifecycle | integration + repro manual orientada + cenário de recovery |

## 1. Mobile Testing Mindset
...
## 2. Common Mobile Testing Anti-Patterns
