...
> Use este arquivo para decidir shell de navegação, retorno e comportamento de deep link.
> Ele é mais útil quando a dúvida é estrutural: tabs, stack, modal, sheet, drawer, rail e back behavior.

---

## Quick Start

- Abra este arquivo quando você precisar escolher ou revisar a estrutura de navegação do app.
- Extraia daqui: padrão de navegação, regra de retorno, comportamento de dismiss e implicações de deep link.
- Se a dúvida já for puramente visual, vá para os guias de plataforma. Se for de performance, vá para `mobile-performance.md`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| escolher o shell principal | `## 1. Navigation Selection Decision Tree` | tabs, stack, drawer, rail ou fluxo linear |
| comparar padrões rapidamente | `## Navigation Pattern Matrix` | quando usar e quando evitar cada padrão |
| revisar comportamento de retorno | `## Back Behavior Checklist` | swipe, hardware back, dismiss e deep link return |
| verificar links profundos | `## 6. Deep Linking` | construção de stack e fallback |

## Navigation Pattern Matrix

| Padrão | Use quando | Evite quando | Impacto no back |
| --- | --- | --- | --- |
| Tabs / Bottom Navigation | 3 a 5 destinos principais, sessões curtas, troca frequente | hierarquia profunda ou destinos demais | cada tab deve preservar sua própria pilha |
| Stack | drill-down, onboarding, checkout, fluxo linear | destinos paralelos de mesma importância | back sobe um nível por vez |
| Modal | tarefa focada e temporária | fluxo principal longo ou navegação recorrente | precisa de dismiss claro e previsível |
| Bottom Sheet | ação contextual, escolha rápida, detalhe complementar | conteúdo longo ou fluxo com muito formulário | swipe-down e fallback visível |
| Drawer | muitos destinos secundários ou enterprise apps | 3 a 5 destinos principais | back não deve mascarar navegação escondida |
| Navigation Rail | tablet/foldable com espaço lateral | phone portrait | comportamento híbrido com master-detail |

## Back Behavior Checklist

- iOS swipe back continua disponível quando o fluxo é de stack comum.
- Android hardware or predictive back retorna contexto, não executa ação arbitrária.
- Modal e bottom sheet têm pelo menos uma saída visível além de gesto.
- Deep link monta trilha de retorno coerente para tela anterior ou raiz segura.
- Fluxos com dados não salvos confirmam descarte sem sequestrar o back para outra ação.

## Common Review Traps

- tabs sem pilha própria por destino
- modal ou sheet sem saída visível
- back hijacked para abrir menu, limpar estado ou trocar tab
- deep link que cai na tela correta, mas sem trilha de retorno
