...
> Use este arquivo quando a decisão depende de toque, gesto, polegar, área clicável ou feedback tátil.
> Ele transforma princípios de ergonomia em decisões concretas de target size, spacing e discoverability.

---

## Quick Start

- Abra este arquivo quando a tela parece correta visualmente, mas ainda pode falhar na mão do usuário.
- Extraia daqui: target size mínimo, spacing, thumb zone, affordance de gesto e uso de haptics.
- Se a dúvida principal é navegação estrutural, volte para `mobile-navigation.md`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| acertar área clicável | `## 1. Fitts' Law for Touch` | tamanho mínimo e distância |
| posicionar CTA ou ações frequentes | `## 2. Thumb Zone Anatomy` | alcance com uma mão |
| revisar gesto e affordance | `## 4. Gesture Psychology` | quando gesto precisa de alternativa visível |
| revisar confirmação tátil | `## 5. Haptic Feedback Patterns` | intensidade e tipo de feedback |

## Touch Decision Guide

- target pequeno quase sempre é bug, não detalhe
- gesto importante precisa de affordance visível ou fallback claro
- CTA frequente compete com alcance do polegar, não com grid perfeito
- haptic feedback serve para confirmar, não para decorar
