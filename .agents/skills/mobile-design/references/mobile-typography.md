...
> Use este arquivo quando tipografia muda leitura, acessibilidade, scaling ou fidelidade de plataforma.
> Ele ajuda a fechar tamanho mínimo, Dynamic Type / text scaling e comportamento em dark mode.

---

## Quick Start

- Abra este arquivo quando a dúvida for legibilidade, escala, font family, text scaling ou contraste de texto em mobile.
- Extraia daqui o mínimo legível para a tela e o que precisa respeitar iOS ou Android.
- Se a dúvida é cor, vá para `mobile-color-system.md`. Se a dúvida é componente nativo, vá para o guia da plataforma.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| estabelecer base legível | `## Typography Decision Guide` | tamanho, line height e weight |
| respeitar iOS | `## 2. System Fonts` e `## 3. Type Scale` | SF Pro, Dynamic Type e hierarchy |
| respeitar Android | `## 2. System Fonts` e `## 3. Type Scale` | Roboto, `sp` e Material scale |
| revisar dark mode e acessibilidade | `## 5. Typography Accessibility` e `## 6. Dark Mode Typography` | contraste e adaptação |

## Typography Decision Guide

- corpo principal precisa começar de uma base legível antes de qualquer finesse visual
- text scaling do usuário faz parte do contrato, não é edge case
- peso regular deve dominar; bold precisa comunicar hierarquia real
- use este arquivo para fechar leitura; use os arquivos de plataforma para fechar aparência nativa
