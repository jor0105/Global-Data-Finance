...
> Use este arquivo quando a solução precisa respeitar comportamento Android, não apenas renderizar no device.
> Ele concentra o que muda em navegação, back, Material 3, tipografia, componentes e feedback.

---

## Quick Start

- Abra este arquivo para revisar decisões que mudam em Android: system bars, predictive back, ripple, Material 3, dynamic color e componentes.
- Extraia daqui o que precisa parecer Android em comportamento, não só em estética.
- Se a dúvida ainda é de shell de navegação, volte para `mobile-navigation.md`. Se a dúvida é iOS-first, vá para `platform-ios.md`.

## Jump Map

| Se você precisa... | Vá para | Decida |
| --- | --- | --- |
| entender o que realmente muda no Android | `## What Changes the Solution on Android` | onde evitar copiar iOS ou web |
| revisar a tela com foco em plataforma | `## Review Checklist` | se o fluxo está Android-correct |
| comparar camadas compartilhadas e divergentes | `## Shared vs Divergent` | o que pode ficar comum e o que exige adaptação |

## What Changes the Solution on Android

- hardware or predictive back precisa manter retorno previsível
- system bars, insets e edge-to-edge mudam layout e CTA fixo
- ripple, snackbar, sheet e FAB carregam expectativa de interação
- Material type scale e dynamic color mudam legibilidade e semântica
- permissões, rationale e reentrada por settings pedem fluxo explícito

## Review Checklist

- system bars e insets respeitados
- text scaling e teclado não quebram a tela
- hardware or predictive back retorna contexto correto
- rationale de permissão faz sentido antes da recusa
- ripple, snackbar ou feedback equivalente confirmam ações
- componentes parecem Android por densidade, hierarchy e motion

## Shared vs Divergent

| Área | Pode ser compartilhada | Precisa divergir no Android quando |
| --- | --- | --- |
| fluxo base | sim, se o objetivo for igual | o retorno, feedback ou componente esperado mudam |
| tipografia | parcialmente | Material scale, `sp` e labels alteram legibilidade |
| navegação | parcialmente | hardware back, bottom nav ou rail exigem comportamento específico |
| feedback | parcialmente | ripple, snackbar e affordances são parte da linguagem da plataforma |
