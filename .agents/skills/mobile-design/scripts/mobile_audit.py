...
8. MOBILE BACKEND (mobile-backend.md):
   ...
   - Offline Handling
   - Retry Path for Error / Offline States
...
class MobileAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        small_sizes = re.findall(
            r'(?:width|height|size):\s*([0-3]\d)', content
        )
        ...
        small_gaps = re.findall(
            r'(?:margin|gap):\s*([0-7])\s*(?:px|dp)', content
        )
        ...
        has_visible_buttons = bool(
            re.search(
                r'Button.*(?:delete|archive|more)|TouchableOpacity|Pressable',
                content,
            )
        )
        ...
        has_important_actions = bool(
            re.search(
                r'(?:onPress|onSubmit|delete|remove|confirm|purchase)', content
            )
        )
        has_haptics = bool(
            re.search(
        ...
        if is_react_native:
            has_pressable = bool(
                re.search(r'Pressable|TouchableOpacity', content)
            )
        ...
        # 1.7 Safe Area / Insets Awareness
        has_safe_area = bool(
            re.search(
                r'SafeAreaView|useSafeAreaInsets|react-native-safe-area-context|SafeArea\(|MediaQuery\.padding|MediaQuery\.viewPadding',
                content,
            )
        )
        has_full_screen_shell = bool(
            re.search(
                r'Scaffold|CupertinoPageScaffold|StatusBar|SystemUiOverlayStyle|NavigationContainer|create[A-Za-z]+Navigator|screenOptions|presentation:|showModalBottomSheet|BottomSheet|Modal',
                content,
            )
        )
        if has_full_screen_shell and not has_safe_area:
            self.warnings.append(
                f'[Safe Area] {filename}: Full-screen mobile UI detected without safe area or inset handling. Content may clash with notch, status bar or home indicator.'
            )

        # 1.8 Keyboard Avoidance
        has_text_input = bool(
            re.search(
                r'TextInput|TextField|TextFormField|EditableText|CupertinoTextField',
                content,
            )
        )
        has_keyboard_avoidance = bool(
            re.search(
                r'KeyboardAvoidingView|KeyboardAwareScrollView|keyboardShouldPersistTaps|Keyboard\.dismiss|resizeToAvoidBottomInset|MediaQuery\.viewInsets|viewInsets\.bottom|SingleChildScrollView',
                content,
            )
        )
        if has_text_input and not has_keyboard_avoidance:
            self.warnings.append(
                f'[Keyboard] {filename}: Text input detected without keyboard avoidance or inset handling. Inputs may be hidden when the virtual keyboard opens.'
            )
        ...
        if is_react_native:
            has_list = bool(
                re.search(r'FlatList|FlashList|SectionList', content)
            )
        ...
        if is_react_native:
            ...
            uses_index_key = bool(
                re.search(r'key=\{.*index.*\}|key:\s*index', content)
            )
        ...
        if is_react_native:
            ...
            has_native_driver = bool(
                re.search(r'useNativeDriver:\s*true', content)
            )
        ...
        if is_react_native:
            ...
            has_subscriptions = bool(
                re.search(
                    r'addEventListener|subscribe|\.focus\(\)|\.off\(', content
                )
            )
        ...
        animating_layout = bool(
            re.search(
                r'Animated\.timing.*(?:width|height|margin|padding)', content
            )
        )
        if animating_layout:
            self.issues.append(
        ...
        tab_bar_items = len(
            re.findall(
                r'Tab\.Screen|createBottomTabNavigator|BottomTab', content
            )
        )
        if tab_bar_items > 5:
            self.warnings.append(
        ...
        has_back_listener = bool(
            re.search(
                r'BackHandler|useFocusEffect|navigation\.addListener', content
            )
        )
        has_custom_back = bool(
            re.search(r'onBackPress|handleBackPress', content)
        )
        ...
        has_linking = bool(
            re.search(
                r'Linking\.|Linking\.openURL|deepLink|universalLink', content
            )
        )
        has_config = bool(
            re.search(
                r'apollo-link|react-native-screens|navigation\.link', content
            )
        )
        ...
        # 3.5 Flutter Return / Dismiss Path Check
        if is_flutter:
            has_route_push = bool(
                re.search(
                    r'Navigator\.push|context\.push|GoRouter|showModalBottomSheet|showDialog',
                    content,
                )
            )
            has_route_pop = bool(
                re.search(
                    r'Navigator\.pop|context\.pop|maybePop|canPop|BackButton|CloseButton',
                    content,
                )
            )
            if has_route_push and not has_route_pop:
                self.warnings.append(
                    f'[Navigation] {filename}: Route or sheet presentation detected without explicit return or dismiss path in this file. Verify back behavior on Android and iOS.'
                )
        ...
        if is_react_native:
            has_custom_font = bool(
                re.search(r"fontFamily:\s*[\"'][^\"']+", content)
            )
        ...
        if re.search(
            r'#000000|color:\s*black|backgroundColor:\s*["\']?black', content
        ):
            self.warnings.append(
        ...
        has_color_schemes = bool(
            re.search(
                r'useColorScheme|colorScheme|appearance:\s*["\']?dark', content
            )
        )
        ...
        if is_react_native:
            ...
            has_ios_icons = bool(
                re.search(r'@expo/vector-icons|ionicons', content)
            )
            ...
            has_haptic_import = bool(
                re.search(
                    r'expo-haptics|react-native-haptic-feedback', content
                )
            )
            has_haptic_types = bool(
                re.search(
                    r'ImpactFeedback|NotificationFeedback|SelectionFeedback',
                    content,
                )
            )
        ...
        has_secure_storage = bool(
            re.search(
                r'SecureStore|Keychain|EncryptedSharedPreferences', content
            )
        )
        ...
        has_network = bool(
            re.search(
                r'fetch|axios|netinfo|@react-native-community/netinfo', content
            )
        )
        has_offline = bool(
        ...
        # 8.2b Retry State Check
        has_retry = bool(
            re.search(
                r'retry|tryAgain|onRetry|refetch|refresh|pullToRefresh|RefreshIndicator',
                content,
                re.IGNORECASE,
            )
        )
        has_error_state = bool(
            re.search(
                r'error|exception|catch\s*\(|snapshot\.hasError|FutureBuilder',
                content,
                re.IGNORECASE,
            )
        )
        if has_network and (has_offline or has_error_state) and not has_retry:
            self.warnings.append(
                f'[Retry] {filename}: Error or offline handling detected without obvious retry path. Mobile users need recovery without restarting the app.'
            )
        ...
        has_push_handler = bool(
            re.search(
                r'onNotification|addNotificationListener|notification\.open',
                content,
            )
        )
        ...
        # 8.4 App Lifecycle Handling
        has_lifecycle_sensitive = bool(
            re.search(
                r'Camera|Video|Audio|Location|Notifications|Firebase\.messaging|WebSocket|socket|StreamSubscription|AppState',
                content,
            )
        )
        has_lifecycle_handling = bool(
            re.search(
                r'AppState|useFocusEffect|applicationDidBecomeActive|applicationWillResignActive|WidgetsBindingObserver|AppLifecycleState|didChangeAppLifecycleState|resumed|inactive|paused|background',
                content,
            )
        )
        if has_lifecycle_sensitive and not has_lifecycle_handling:
            self.warnings.append(
                f'[Lifecycle] {filename}: Lifecycle-sensitive feature detected without clear background or resume handling. Verify app state transitions and recovery after interruption.'
            )
        ...
        if is_react_native:
            ...
            _has_large_title = bool(
                re.search(
                    r'fontSize:\s*34|largeTitle|font-weight:\s*["\']?bold',
                    content,
                )
            )
            ...
            _has_body = bool(
                re.search(r'fontSize:\s*17.*regular|body', content)
            )
            ...
            matching_ios = sum(
                ...
                if any(
                    abs(float(size) - ios_size) < 1
                    for ios_size in ios_scale_sizes
                )
            )
        ...
        if is_react_native:
            ...
            _has_display = bool(
                re.search(r'fontSize:\s*[456][0-9]|display', content)
            )
            _has_headline_material = bool(
                re.search(r'fontSize:\s*[23][0-9]|headline', content)
            )
            _has_title_material = bool(
                re.search(r'fontSize:\s*2[12][0-9].*medium|title', content)
            )
            _has_body_material = bool(
                re.search(r'fontSize:\s*1[456].*regular|body', content)
            )
            has_label = bool(
                re.search(r'fontSize:\s*1[1234].*medium|label', content)
            )
        ...
        has_dark_mode = bool(
            re.search(
                r'dark:\s*|isDark|useColorScheme|colorScheme:\s*["\']?dark',
                content,
            )
        )
        if has_dark_mode:
        ...
        if is_react_native:
            ...
            has_sf_pro = bool(
                re.search(
                    r'SF Pro|SFPro|fontFamily:\s*["\']?[-\s]*SF', content
                )
            )
            has_custom_font = bool(
                re.search(r'fontFamily:\s*["\'][^"\']+', content)
            ...
            ios_green = bool(
                re.search(r'#34C759|#30D158|systemGreen', content)
            )
            ios_red = bool(re.search(r'#FF3B30|#FF453A|systemRed', content))

            has_custom_primary = bool(
                re.search(
                    r'primaryColor|theme.*primary|colors\.primary', content
                )
            )
            ...
            has_header_title = bool(
                re.search(
                    r'title:\s*["\']|headerTitle|navigation\.setOptions',
                    content,
                )
            )
            ...
            has_action_sheet = bool(
                re.search(
                    r'ActionSheet|ActionSheetIOS|showActionSheetWithOptions',
                    content,
                )
            )
        ...
        if is_react_native:
            ...
            has_material_colors = bool(
                re.search(
                    r'MD3|MaterialYou|dynamicColor|useColorScheme', content
                )
            )
            has_theme_provider = bool(
                re.search(
                    r'MaterialTheme|ThemeProvider|PaperProvider|ThemeProvider',
                    content,
                )
            )
            ...
            has_snackbar = bool(
                re.search(r'Snackbar|showSnackBar|Toast', content)
            )
            ...
            has_bottom_nav = bool(
                re.search(r'BottomNavigation|BottomNav', content)
            )
            has_navigation_rail = bool(re.search(r'NavigationRail', content))

            if has_bottom_nav:
                self.passed_count += 1  # Good Android pattern
            elif has_top_app_bar and not (
                has_bottom_nav or has_navigation_rail
            ):
                self.warnings.append(
        ...
        has_rntl = bool(
            re.search(
                r'react-native-testing-library|@testing-library', content
            )
        )
        has_detox = bool(
            re.search(r'detox|element\(|by\.text|by\.id', content)
        )
        ...
        test_files = len(
            re.findall(r'\.test\.(tsx|ts|js|jsx)|\.spec\.', content)
        )
        e2e_tests = len(
            re.findall(r'detox|maestro|e2e|spec\.e2e', content.lower())
        )
        ...
        if is_react_native:
            has_pressable = bool(
                re.search(
                    r'Pressable|TouchableOpacity|TouchableHighlight', content
                )
            )
            has_a11y_label = bool(
        ...
        _has_debugger = bool(
            re.search(r'debugger|__DEV__|React\.DevTools', content)
        )
