...
Leia quando:
- voce precisa escolher ou revisar paleta, contraste, modo claro/escuro ou cor de destaque
- a decisao pede direcao de cor, nao apenas trocar um token ja definido

Extraia:
- familia de cor adequada ao contexto
- distribuicao 60-30-10 e contraste minimo aceitavel
- como gerar variacoes via HSL sem repetir defaults genericos
- anti-patterns que tornam a interface previsivel ou fora de contexto
...
| **Purple** | Creative, premium, expressive when context supports it | Overused as an AI default; justify it with brand and content instead of using it by reflex |
...
4. **CONFIRM ONLY IF MATERIAL** → Ask only when brand preference cannot be inferred and the choice would materially change the output
...
- [ ] **Preference inferred or confirmed?** (only ask if the choice is materially ambiguous)
- [ ] **Assumption explicit when inferred?** (if brand direction was not provided)
...
- Default to purple/violet without contextual reason
...
- Turn every palette choice into a user question

### ✅ DO:

- Generate fresh palette per project
- State assumptions when color direction was inferred from context
...
## 10. Emotion Color Mapping

Use este mapa como heuristica de direcao, nao como tabela fixa de hex codes:

```
┌────────────────────────────────────────────────────┐
│  EMOTION          │  FAMILIAS         │  USO       │
├───────────────────┼───────────────────┼────────────┤
...
Converta a familia emocional em paleta contextual usando os passos deste
arquivo, nao copiando uma cor pronta por intuicao.
