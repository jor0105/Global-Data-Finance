...
|---|---|---|---|---|
| `/opsx:new` ou "cria uma change" | `.agents/workflows/opsx-new.prompt.md` | Não há seleção prévia; derive ou valide o nome da nova change | Default schema, salvo pedido explícito de outro schema | Para depois de mostrar o primeiro artefato `ready` |
...
| `/opsx:ff` ou "gera tudo até apply-ready" | `.agents/workflows/opsx-ff.prompt.md` | Nova change; se o nome já existir, confirmar continuidade | Default schema ou schema explicitado pelo usuário | Para quando `apply.requires` estiver satisfeito |
