...
def is_frontend_project(project_path: Path) -> bool:
    ...
    # Basic check: if there are any HTML/JSX/TSX files outside of skip_dirs
    files = find_html_files(project_path, skip_check=True)
    return len(files) > 3


def find_html_files(project_path: Path, skip_check: bool = False) -> list:
...
def check_accessibility(file_path: Path) -> list:
    ...
    try:
        ...
        buttons = re.findall(
            r'<button[^>]*>[^<]*</button>', content, re.IGNORECASE
        )
        for btn in buttons:
        ...
        onkeydown_count = content.lower().count(
            'onkeydown='
