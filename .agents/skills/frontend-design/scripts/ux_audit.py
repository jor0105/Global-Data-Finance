...
class UXAuditor:
    def __init__(self, project_root: str | None = None):
        ...
        self.allowed_purple_terms = self._load_allowed_purple_terms()
    ...
    def _find_project_root(self, start: str) -> Path:
        ...
        for candidate in (current, *current.parents):
            if (candidate / 'AGENTS.md').exists() or (
    ...
    def _load_allowed_purple_terms(self) -> set[str]:
        """Read project-level color policy so repo tokens do not become false positives."""
        policy_files = [
        ...
        allowed: set[str] = set()
        if (
            'cta accent: violet' in policy_text
            or '--color-cta: #8b5cf6' in policy_text
        ):
            allowed.update({'#8b5cf6', 'violet'})
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_markup_form = bool(
            re.search(
                r'<form\b|<input\b|<select\b|<textarea\b|type=["\']password|credit[-_\s]?card|payment|checkout',
                content,
                re.IGNORECASE,
            )
        )
        has_form = has_markup_form and not is_stylesheet
        complex_elements = (
            0
            if is_stylesheet
            else len(
        ...
        if re.search(r'height:\s*([0-3]\d)px', content) or re.search(
            ...
            self.warnings.append(
                f"[Fitts' Law] {filename}: Small targets (< 44px)"
            )
        ...
        if 'onClick' in content or '@click' in content or 'onclick' in content:
            ...
            has_state_change = re.search(
                r'setState|useState|disabled|loading', content
            )
        ...
        # Visual noise check
        has_many_colors = (
            len(re.findall(r'#[0-9a-fA-F]{3,6}|rgb|hsl', content)) > 15
        )
        ...
        if has_form:
            has_standard_labels = bool(
                re.search(
                    r'<label|placeholder|aria-label', content, re.IGNORECASE
                )
            )
        ...
        if has_form:
            has_defaults = bool(
                re.search(
                    r'checked|selected|default|value=["\'].*["\']', content
                )
            )
            radio_inputs = len(
                re.findall(r'type=["\']radio', content, re.IGNORECASE)
            )
        ...
        if re.search(r'price|pricing|cost|\$\d+', content, re.IGNORECASE):
            has_anchor = bool(
                re.search(
                    r'original|was|strike|del|save \d+%',
                    content,
                    re.IGNORECASE,
                )
            )
        ...
        if has_form:
            has_progress = bool(
                re.search(
                    r'progress|step \d+|complete|%|bar', content, re.IGNORECASE
                )
            )
        ...
        if text_elements > 0 and not re.search(
            r'leading-|line-height:', content
        ):
            self.warnings.append(
        ...
        if re.search(
            ...
            line_heights = re.findall(
                r'(?:leading-|line-height:\s*)([\d.]+)', content
            )
        ...
        font_sizes = re.findall(
            r'font-size:\s*(\d+(?:\.\d+)?)(px|rem|em)', content
        )
        size_values = []
        ...
        has_gradient = bool(
            re.search(
                r'gradient|linear-gradient|radial-gradient|conic-gradient',
                content,
            )
        )
        if has_gradient:
            ...
            gradient_count = len(
                re.findall(r'gradient', content, re.IGNORECASE)
            )
            if gradient_count > 5:
                self.warnings.append(
        ...
        has_images = bool(
            re.search(r'<img|background-image:|bg-\[url', content)
        )
        ...
        # 4.1 Purple policy - respect project tokens before flagging generic purple drift
        purple_hexes = [
            '#8B5CF6',
            '#A855F7',
            '#9333EA',
        ...
        for purple in purple_hexes:
            normalized = purple.lower()
            if normalized in self.allowed_purple_terms:
                continue
            if normalized in content.lower():
                self.issues.append(
                    f"[Color] {filename}: Unapproved purple token detected ('{purple}'). Use only project-approved CTA violet tokens or switch to Amber/Emerald/Slate."
                )
                break
        ...
        if total_colors > 3:
            ...
            bg_declarations = re.findall(
                r'(?:background|bg-|bg\[)([^;}\s]+)', content
            )
            text_declarations = re.findall(
                r'(?:color|text-)([^;}\s]+)', content
            )
        ...
        has_hover_focus = bool(
            re.search(r'hover:|focus:|:hover|:focus', content)
        )
        ...
        has_loading_indicator = bool(
            re.search(
                r'skeleton|spinner|progress|loading|<circle.*animate', content
            )
        )
        ...
        has_routing = bool(
            re.search(r'router|navigate|Link.*to|useHistory', content)
        )
        has_page_transition = bool(
            re.search(
                r'AnimatePresence|motion\.|transition.*page|fade.*route',
                content,
            )
        )
        ...
        has_3d_transform = bool(
            re.search(
                r'transform3d|perspective\(|rotate3d|translate3d', content
            )
        )
        if has_3d_transform:
        ...
        if total_animations > 5:
            ...
            functional_animations = len(
                re.findall(
                    r'hover:|focus:|disabled|loading|error|success', content
                )
            )
        ...
        if re.search(r'<img(?![^>]*alt=)[^>]*>', content):
            self.issues.append(
                f'[Accessibility] {filename}: Missing img alt text'
            )

    def is_frontend_project(self, directory: str) -> bool:
        """Check if the directory is likely a frontend project."""
        indicators = {
            'package.json',
            'node_modules',
            'next.config.js',
        ...
        for _root, dirs, files in os.walk(directory):
            ...
            for file in files:
                if Path(file).suffix in extensions:
                    frontend_files += 1
                if frontend_files > 3:  # Reasonable threshold for "frontend"
                    return True
        return False

    def audit_directory(self, directory: str) -> None:
        if not self.is_frontend_project(directory):
            print(
                f'Skipping UX audit: No frontend root detected in {directory}'
            )
            return
        ...
        for root, dirs, files in os.walk(directory):
            ...
            for file in files:
                if any(marker in file for marker in ('.test.', '.spec.')):
                    continue
                if Path(file).suffix in extensions:
