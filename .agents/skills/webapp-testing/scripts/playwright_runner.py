...
def run_basic_test(target_url: str, screenshot_enabled: bool = False) -> dict:
    ...
    try:
        with sync_playwright() as p:
            ...
            if screenshot_enabled:
                ...
                test_result['screenshot_note'] = (
                    'Saved to temp directory (auto-cleaned by OS)'
                )

            # Element counts
            test_result['elements'] = {
            ...
            test_result['status'] = (
                'success' if test_result['health']['loaded'] else 'failed'
            )
            test_result['summary'] = (
        ...
        test_result['summary'] = f'[X] Error: {str(e)[:100]}'

    return test_result


def run_accessibility_check(target_url: str) -> dict:
    """Run basic accessibility check."""
    if not PLAYWRIGHT_AVAILABLE:
        return {'error': 'Playwright not installed'}

    results = {'url': target_url, 'accessibility': {}}
