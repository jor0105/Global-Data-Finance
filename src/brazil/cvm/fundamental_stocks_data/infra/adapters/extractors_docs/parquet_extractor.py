...
from src.brazil.cvm.fundamental_stocks_data.application import FileExtractorRepository
...
class ParquetExtractor(FileExtractorRepository):
    ...
    def __init__(
        ...
        logger.debug(f"ParquetExtractor initialized with chunk_size={self.chunk_size}")
    ...
    def _extract_with_transaction(self, zip_path: str, output_dir: str) -> None:
        ...
        try:
            with zipfile.ZipFile(zip_path, "r") as z:
                csv_files = Extractor.list_files_in_zip(zip_path, ".csv")
        ...
        for encoding in self.encodings:
            try:
                with zip_file.open(csv_filename) as csv_file:
                    pd.read_csv(
                        BytesIO(csv_file.read(10000)),  # Test first 10KB
                        encoding=encoding,
                        sep=";",
                        on_bad_lines="skip",
                        nrows=100,
