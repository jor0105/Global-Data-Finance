...
def dados_opcoes_b3(taxa_selic_atual):
    ...
    df_opcoes_b3['Data Vencimento'] = pd.to_datetime(
        df_opcoes_b3['Data Vencimento']
    )
    ...
    vencimentos_np = df_opcoes_b3['Data Vencimento'].values.astype(
        'datetime64[D]'
    )
    ...
    df_dados_vol_acoes_b3['Preço Ação'] = [
        ultimo_fechamento_por_codigo(c)
        for c in df_dados_vol_acoes_b3['Código']
    ]
    ...
    colunas_final = list(df_opcoes_b3.columns) + [
        'Vol_Historica',
        'Preço Ação',
    ]
    ...
    df_opcoes_b3['Código'] = df_opcoes_b3['Código'] + df_opcoes_b3[
        'Tipo Ação'
    ].map(mapeamento).fillna(0).astype(int).astype(str)
