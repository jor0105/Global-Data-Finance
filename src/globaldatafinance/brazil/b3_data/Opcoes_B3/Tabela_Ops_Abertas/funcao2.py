...
def dados_opcoes_br2() -> pd.DataFrame:
    ...
    df['Preço Exercício'] = pd.to_numeric(
        df['Preço Exercício'], errors='coerce'
    )
    df['Spread Travas, Cobertas e Descobertas'] = (
        df['Posições Travadas']
        + df['Posições Cobertas']
        - df['Posições Descobertas']
    )
    df['Diferença entre vendedores e compradores'] = (
        df['Titulares'] - df['Lançadores']
    )
