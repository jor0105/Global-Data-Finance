...
def concatenar_docs(
    ...
    if not data.empty:
        ...
        data.loc[mask_na, 'Código'] = data.loc[
            mask_na, 'Nome_Empresarial'
        ].map(mapa_nome)
        ...
        data['Data_Referencia'] = data['Data_Referencia'].dt.strftime(
            '%Y-%m-%d'
        )
        data.to_parquet(
    ...
    if not data.empty:
        ...
        data['Código'] = data['CNPJ_Companhia'].map(mapa_cnpj)
        mask_na = data['Código'].isna()
        data.loc[mask_na, 'Código'] = data.loc[mask_na, 'Empresa'].map(
            mapa_nome
        )
        data = data.dropna(subset=['Código'])
    ...
    for nome in nomes_arquivos_fca:
            ...
            try:
                ...
                if not arquivo.empty:
                    ...
                    if nome == 'fca_cia_aberta_geral_':
                        ...
                        arquivo = arquivo.sort_values(
                            by='Versao', ascending=False
                        )
                        ...
                        arquivo.drop(
                            ['Versao', 'Sigla_UF'], axis=1, inplace=True
                        )
                        ...
                        arquivo = arquivo.drop_duplicates(
                            subset=['Tipo_Endereco', 'Complemento'],
                            keep='first',
                        )
                        arquivo.drop(
                            [
        ...
        if not data.empty:
            ...
            data.sort_values(
                by='Data_Referencia', ascending=False, inplace=True
            )
            data.to_parquet(
    ...
    for nome in nomes_arquivos_fre:
            ...
            try:
                ...
                if not arquivo.empty:
                    ...
                    if nome == 'fre_cia_aberta_membro_comite_':
                        arquivo['Percentual_Participacao_Reunioes'] = (
                            pd.to_numeric(
                                arquivo['Percentual_Participacao_Reunioes'],
                                errors='coerce',
                            )
                        ...
                        arquivo = arquivo[
                            pd.to_numeric(
                                arquivo['Numero_Membros_Remunerados'],
                                errors='coerce',
                            )
                            > 0
                        ]
                        ...
                        arquivo.drop(
                            ['ID_Capital_Social'], axis=1, inplace=True
                        )
                        ...
                        arquivo['Percentual_Acao_Ordinaria_Circulacao'] = (
                            arquivo[
                        ...
                        arquivo['Percentual_Acao_Preferencial_Circulacao'] = (
                            arquivo[
        ...
        try:
            if not data.empty:
                ...
                mask_na = data['Código'].isna()
                data.loc[mask_na, 'Código'] = data.loc[
                    mask_na, 'Nome_Companhia'
                ].map(mapa_nome)
                data = data.dropna(subset=['Código'])
                data.drop(
                    ['Nome_Companhia', 'CNPJ_Companhia'], axis=1, inplace=True
                )
