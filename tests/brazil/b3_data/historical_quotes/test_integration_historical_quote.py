...
def cotahist_path() -> str:
    """Return the configured local COTAHIST dataset path."""
    configured_path = os.environ.get('COTAHIST_PATH')
    if not configured_path:
        pytest.skip('COTAHIST_PATH is not configured for integration tests')

    assert configured_path is not None
    dataset_path = Path(configured_path).expanduser()
    if not dataset_path.is_dir():
        pytest.skip(f'COTAHIST_PATH is not a directory: {dataset_path}')
...
class TestFastModeExtraction:
    ...
    def test_fast_mode_extraction(self, historical_quotes, cotahist_path):
        with tempfile.TemporaryDirectory() as temp_dir:
            result = historical_quotes.extract(
                path_of_docs=cotahist_path,
            ...
            assert result['total_records'] >= 0, (
                'total_records should be non-negative'
            )
            assert (
    ...
    def test_slow_mode_extraction(self, historical_quotes, cotahist_path):
        with tempfile.TemporaryDirectory() as temp_dir:
            result = historical_quotes.extract(
                path_of_docs=cotahist_path,
                destination_path=temp_dir,
    ...
    def test_multiple_assets(self, historical_quotes, cotahist_path):
        with tempfile.TemporaryDirectory() as temp_dir:
            result = historical_quotes.extract(
                path_of_docs=cotahist_path,
                destination_path=temp_dir,
    ...
    def test_multiple_years_and_assets(self, historical_quotes, cotahist_path):
        with tempfile.TemporaryDirectory() as temp_dir:
            result = historical_quotes.extract(
                path_of_docs=cotahist_path,
                destination_path=temp_dir,
            ...
            assert result['total_files'] >= 0, (
                'Multiple years should process files'
            )
    ...
    def test_invalid_processing_mode(self, historical_quotes):
        with (
            ...
            pytest.raises(
                InvalidProcessingMode, match='Invalid processing_mode'
            ),
        ):
            historical_quotes.extract(
                path_of_docs=temp_dir,
                destination_path=temp_dir,
                assets_list=['ações'],
                initial_year=2024,
                last_year=2024,
    ...
    def test_output_filename_without_extension(
        self, historical_quotes, cotahist_path
    ):
        with tempfile.TemporaryDirectory() as temp_dir:
            result = historical_quotes.extract(
                path_of_docs=cotahist_path,
    ...
    def test_output_filename_with_extension(
        self, historical_quotes, cotahist_path
    ):
        with tempfile.TemporaryDirectory() as temp_dir:
            result = historical_quotes.extract(
            ...
            if result['total_files'] > 0:
                assert result['output_file'].endswith(
                    'test_with_ext.parquet'
