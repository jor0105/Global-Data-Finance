...
class TestZipCleanup:
    ...
    def test_zip_deleted_after_successful_extraction(self, tmp_path, mock_zip):
        ...
        extractor.extract(
            source_path=str(mock_zip), destination_path=str(tmp_path)
        )
    ...
    def test_integration_with_httpx_adapter(self, tmp_path):
        ...
        if parquet_files:
            ...
            assert downloaded_zip.exists(), (
                'ZIP was deleted without creating parquets'
            )
    ...
    def test_cleanup_does_not_delete_other_zips(self, tmp_path):
        ...
        for zip_path in [zip1, zip2, zip3]:
            ...
            with zipfile.ZipFile(zip_path, 'w') as z:
                z.writestr(
                    'data.csv',
                    data.to_csv(sep=';', index=False).encode('latin-1'),
                )
        ...
        extractor = ParquetExtractorAdapterCVM()
        extractor.extract(
            source_path=str(zip1), destination_path=str(output_dir)
        )
