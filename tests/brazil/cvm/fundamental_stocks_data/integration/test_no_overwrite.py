...
class TestNoOverwrite:
    ...
    def test_multiple_extraction_attempts_overwrite_original(self, tmp_path):
        ...
        print(
            '✅ Data updated to the latest version after 5 extraction attempts'
        )
    ...
    def test_concurrent_extractions_overwrite(self, tmp_path):
        ...
        extractor = ParquetExtractorAdapterCVM()
        extractor.extract(
            source_path=str(zip1), destination_path=str(tmp_path)
        )
        extractor.extract(
            source_path=str(zip2), destination_path=str(tmp_path)
        )
