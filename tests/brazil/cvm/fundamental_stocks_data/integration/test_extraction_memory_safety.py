...
class TestMemorySafety:
    ...
    def test_extraction_does_not_load_full_file_in_memory(
        ...
        print(f"✅ Extraction with controlled memory: {len(df_result)} rows processed")
    ...
    def test_multiple_large_files_do_not_accumulate_memory(self, tmp_path):
        ...
        for i, zip_file in enumerate(zip_files):
            Extractor.extract_zip_to_parquet(
                chunk_size=10_000, zip_path=str(zip_file), output_dir=str(output_dir)
            )
