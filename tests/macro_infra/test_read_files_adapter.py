...
class TestReadFilesAdapter:
	...
	def test_read_csv_test_encoding_detects_any_supported_encoding(self, tmp_path, content):
		csv_name = "test.csv"
	...
	def test_read_csv_test_encoding_detects_supported_encoding_with_non_ascii(self, tmp_path):
	...
	def test_read_csv_test_encoding_logs_and_skips_bad_encoding(self, tmp_path, caplog):
		...
		with zipfile.ZipFile(zip_path, "r") as zf:
			with caplog.at_level('DEBUG'):
				detected = ReadFilesAdapter.read_csv_test_encoding(zf, csv_name)
		assert detected in ["utf-8", "latin-1", "iso-8859-1", "cp1252"]
		assert any("Validated" in r.message or "Test read failed" in r.message for r in caplog.records)
