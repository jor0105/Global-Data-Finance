...
- `DownloadResultCVM`: Structured result object containing:
  ...
  - `successful_downloads` (`list[str]`): List of downloaded file identifiers.
  - `failed_downloads` (`dict[str, str]`): Mapping of failed file identifiers to failure messages.
...
- `output_filename` (`str`): Base output filename for the created Parquet file (must be a basename without path separators or `.parquet` extension). Default: `"cotahist_extracted"`.
...
- `ExtractionResultB3` (`TypedDict`): A diagnostic mapping containing runtime confirmation metrics:
  ...
  - `total_files` (`int`): Total count of ZIP archives processed.
  - `success_count` (`int`): Count of successfully parsed archives.
  - `error_count` (`int`): Count of archives that failed during processing.
  ...
  - `errors` (`list[str]`, optional): Error diagnostics if issues occurred.
