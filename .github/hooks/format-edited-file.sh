...
if [[ ! -x node_modules/.bin/prettier ]]; then
  echo "[hook] prettier skipped: local dependency is not installed"
  exit 0
fi
...
for file in "${candidates[@]}"; do
  ...
  case "$file" in
    node_modules/*|dist/*|.venv/*|.agents/*|.opencode/*|.kilo/*)
      continue
      ;;
  esac
...
if (( ${#files[@]} == 0 )); then
  echo "[hook] prettier skipped: no supported edited file detected"
  exit 0
fi

printf "[hook] prettier focused format:"
printf ' %q' "${files[@]}"
printf '\n'
